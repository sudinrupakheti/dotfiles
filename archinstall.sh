...
check_status "Installing btrfs-progs"
...
echo -e "${YELLOW}Formatting Root partition as $FS_TYPE...${RESET}"
...
[[ "$ZRAM_ENABLE" == "n" && -n "$SWAP" ]] && mkfs -t swap "$SWAP"
...
if [[ "$FS_TYPE" == "btrfs" ]]; then
    ...
    mkdir -p /mnt/home /mnt/var/log /mnt/var/cache
